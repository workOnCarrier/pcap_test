...
docker run --rm -v "$PWD/data:/app/data" pcap-learn /app/data
```
...
- Exchange sequence numbers are read as 32-bit little-endian integers at the start of each UDP payload, per the CME SBE header spec.
...
- Future enhancements could include mmap-based pcap parsing, multi-threaded decoding, richer telemetry, and more extensive statistics. On the architecture front, instead of passing aggregator, we could use a eventbus so new aggreators can be added without having to change the parsing code
