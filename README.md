...
```bash
docker build -t flow-traders .
docker run --rm -v "$PWD:/data" flow-traders /data
```

Mount any directory containing pcaps at `/data` (or pass an explicit directory path after the image name).

## Design notes & assumptions

- Packets are parsed from Ethernet/IPv4/UDP frames with an appended 20-byte Metamako trailer. Timestamp seconds/nanoseconds are treated as big-endian as observed in the provided data.
